(function(){
	...
	//valor do placar
	var placar = document.querySelector("#valor");

	//numero de jogadas
	var nJogadas = document.querySelector("#num");
	...
	//variavel que armazera o valor do placar
	var valor=20;

	//variavel que armazena o numero de jogadas
	var num=0;
	...
	var cont0=0, cont1=0, cont2=0, cont3=0;
	...
	function opp(i){
		if(i%4 === 0) return 10 + (cont0 * 210) + "px";
		if(i%4 === 1) return 10 + (cont1 * 210) + "px";
		if(i%4 === 2) return 10 + (cont2 * 210) + "px";
		if(i%4 === 3) return 10 + (cont3 * 210) + "px";
	}
	...
	function startGame(){
		...
		//reinicia o valor do placar e o numero de jogadas
		valor=20;
		num=0;
		placar.innerHTML = valor;
		nJogadas.innerHTML = num;

		//zera os contadores que organizam as cartas
		cont0=0, cont1=0, cont2=0, cont3=0;
		...
		//	card.style.left = (i % 4) === 0 ? 10 + "px" : 10 + ((i % 4) * 170) + "px";
		//	card.style.top = i/4 >= 1 ? 260 + "px" : 10 + "px";
			card.style.left = opp(i);
			card.style.top = op(i);
			...
			frontFaces[i].style.backgroundSize = "200px 200px";
		...
		if(flippedCards.length < 2){
			...
			if(flippedCards.length === 2){
				...
				if(flippedCards[0].childNodes[3].id === flippedCards[1].childNodes[3].id){
					...
					//atualiza o placar e ++ no numero de jogadas
					attPlacar(true);
					...
					setTimeout(function(){
						//caso haja duas cartas no array de cartas viradas, o timer remove a classe flipped das cartas no array de cartas viradas após 1,5 segundos
	...
	//funcao que atualiza o placar e num de jogadas
	function attPlacar(x){
		if(x){
			valor += 20;
			num++;
			placar.innerHTML = valor;
	...
	function matchCardsSign(){
		//deixa o match visivel
		matchSign.style.visibility = "visible";
		...
		setTimeout(function(){
			//deixa o match invisivel
			matchSign.style.visibility = "hidden";
